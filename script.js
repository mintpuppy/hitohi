...
(function () {
  ...
  function openSheet(title, buildBody, opts) {
    ...
    if (opts.anchor) {
      ...
      var below = window.innerHeight - r.bottom - 22;
      var above = r.top - 22;
      var useBelow = below >= 280 || below >= above;
      var w = Math.min(390, window.innerWidth - 24);
      el.sheetLayer.className = 'sheet-layer is-anchored';
      sheet.className = 'sheet is-anchored' + (useBelow ? '' : ' is-above') +
        (opts.align === 'left' ? ' is-left' : '');
      if (opts.align === 'left') {
        sheet.style.left = Math.round(Math.max(12, Math.min(r.left, window.innerWidth - w - 12))) + 'px';
      } else {
        sheet.style.right = Math.round(Math.max(12, window.innerWidth - r.right)) + 'px';
      }
      if (useBelow) {
        sheet.style.top = Math.round(r.bottom + 10) + 'px';
        sheet.style.maxHeight = Math.round(Math.min(below, 560)) + 'px';
      } else {
        sheet.style.bottom = Math.round(window.innerHeight - r.top + 10) + 'px';
        sheet.style.maxHeight = Math.round(Math.min(above, 560)) + 'px';
      }
    } else {
      el.sheetLayer.className = 'sheet-layer';
    }
  ...
  function render() {
    ...
    if (!stepPinned) activeStep = suggestedStep(p, status);
  ...
  /* TODO が1つもない日は必ず LIST から。1つでも入っていれば、その日に最後に開いていた
     ステップ（なければ進み具合から決まるステップ）を続きとして開く。 */
  function suggestedStep(p, status) {
    if (!p.todos.length) return 'list';
    if (STEP_KEYS.indexOf(p.step) >= 0) return p.step;
    return !status.list ? 'list' : (!status.top3 ? 'top3' : (!status.time ? 'time' : 'done'));
  }
  ...
  function renderSlots(p) {
    ...
    [1, 2, 3].forEach(function (rank) {
      ...
      btn.addEventListener('click', function () { openRankSheet(rank, btn); });
  ...
  function renderTodos(p) {
    ...
    list.forEach(function (todo) {
      ...
      more.textContent = '…';
      more.addEventListener('click', function () { openTodoMenu(todo.id, 'list'); });
      li.appendChild(more);
    ...
    HOURS.forEach(function (hour) {
      ...
      placed.filter(function (t) { return hourOf(t.scheduledTime) === hour; }).forEach(function (todo) {
        ...
        if (editingBlockId === todo.id) {
          ...
          text.addEventListener('click', function () { openTodoMenu(todo.id, 'block'); });
  ...
  function openTimeSheet(id) {
    ...
    openSheet('TIME に置く　' + todo.text, function (body) {
      ...
      if (todo.scheduledTime) {
        sheetItem(body, 'TIME から外す（TODO は残る）', {
          onTap: function () {
            setTime(p, id, null);
            save();
            closeSheet();
    ...
    openSheet(todo.text, function (body) {
      if (mode === 'block') {
        ...
        sheetItem(body, 'TODO を修正', {
          onTap: function () { closeSheet(); editingId = null; editingBlockId = id; render(); }
        });
      }
  ...
  function init() {
    ...
    el.steps.addEventListener('click', function (e) {
      ...
      stepPinned = true;
      document.body.dataset.step = activeStep;
      if (page().todos.length) { livePage().step = activeStep; save(); }
      render();
      if (activeStep === 'list') {
